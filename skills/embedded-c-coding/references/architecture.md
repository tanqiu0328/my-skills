...
- 指导原则与适用边界
- 封装、回调和接口隔离
- 项目五层架构与 Platform 设备模型
- 自动初始化与 `container_of`
- 非阻塞驱动和交付检查

## 指导原则

先遵循项目现有架构。只有模块存在复杂状态、可替换实现、公共生命周期
或需要稳定测试 seam 时，才使用 SOLID 和面向对象式封装。
关于适用的 C 设计模式，参阅 [design-patterns.md](design-patterns.md)。
...
普通计算、简单适配和单一硬件操作可以保持为直接函数和数据结构。
当封装能隐藏真实复杂度时，再使用不透明类型、函数指针或对象式生命周期。
...
当驱动需要异步通知状态变化（错误、完成、数据就绪）时，
使用项目现有的回调、事件或消息机制。周期性采样或受控轮询已经是
项目执行模型的一部分时，可以继续使用有界轮询。
...
仅当仓库已经采用以下五层结构时应用本节；其他项目遵循其现有边界：
...
当上层采用事件驱动或 RTC 模型，而底层操作可能超过允许执行预算时，
驱动提供有界异步行为。优先复用项目已有 DMA、ISR、状态机、工作队列
或专用线程；不要仅为统一形式创建新线程。
...
- 公共API在入队后立即返回
- 需要专用工作线程时，由明确的资源所有者创建并配置栈和优先级
- 周期性轮询放在项目约定的调度上下文中，并记录周期与最坏执行时间
- 结果和错误通过回调传递
- 应用层无需了解工作线程的存在
- 与业务无关的内部细节不在头文件中暴露
