...
先测量栈高水位并选择静态缓冲区、固定容量对象池或调整任务栈；
只有项目允许运行时动态分配且能够接受失败和碎片风险时才使用堆。
...
3. **按相反顺序释放** - 如果A分配了B、B分配了C，
   则先释放C再释放B再释放A
4. **清除所有者引用** - 释放后清除拥有该对象的指针；同时检查其他别名
...
消息队列通常能把共享状态收敛到一个受控 seam。仍需明确消息对象的
所有权、复制/引用语义、队列满策略和发送接收上下文：
...
1. 优先使用静态缓冲区、固定容量内存池或调整任务栈；只有项目允许运行时动态分配且所有权明确时才使用堆
2. 避免递归，使用迭代算法
3. 测试时监控栈高水位标记
4. 根据实际测量用量加余量配置栈大小
...
详细的防御性编程模式（包括 const 正确性和错误处理策略），
参见 [clean-code.md](clean-code.md)。
