...
description: 软件架构设计并维护 SWE.2 证据。用户要求划分软件组件、定义接口或动态行为、分析 ROM、RAM、CPU、时序、并发及架构决策时使用
---
...
7. 创建或更新 `software-architecture.md`，执行交付检查并报告路径、决策和缺口
   - 优先使用小型 Mermaid 图呈现三个以上组件的结构或交互
   - 完成标准：契约中的每项交付检查均通过，受影响方可据此开展详细设计
